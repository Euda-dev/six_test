...
<div class="col-md-12">
    <form action="{{ route('edit_products_post', ['id' => $product->id]) }}" method="post" onsubmit="return checkForm()">
        @csrf
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          <label for="value">Valor do Produto <span style="color: red">*</span></label>
            <input type="text" id="value" name="value" class="form-control" pattern="\d+(\.\d{2})?" value="{{$product->value}}" title="Informe um número com duas casas decimais, use ponto e não vírgula!">
        </div>
        <div class="form-group">
          <label for="quantity">Quantidade no estoque <span style="color: red">*</span></label>
          <input type="number" id="quantity" name="quantity" class="form-control" value="{{$product->stocks->quantity}}">
        </div>
      </div>
      <!-- /.card-body -->
    </div>
